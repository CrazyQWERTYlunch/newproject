...
from homeworks_app3.models import Order, Product, OrderItem, Client
...
def view_orders(request, client_id=None):
    if client_id:
        ...
        return render(request, 'homeworks_app3/get_orders.html', context=context)
    else:
        return render(request, 'homeworks_app3/get_orders.html', {'title': f'Заказы', 'orders': Order.objects.all()})
...
def view_ordered_goods(request, client_id=None, start_date=None):
    ...
    return render(request, 'homeworks_app3/ordered_goods.html', context=context)
...
def product(request, product_slug=False):
    ...
    return render(request, 'homeworks_app3/product.html', context=context)
